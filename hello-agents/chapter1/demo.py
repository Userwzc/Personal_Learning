...
def get_attraction(city: str, weather: str) -> str:
    """
    根据城市和天气使用Travily Search API搜索并返回优化后的景点推荐。
    """
    # 1. 从环境变量中读取API密钥
    api_key = os.getenv("TAVILY_API_KEY")
    if not api_key:
        return "错误：未找到Tavily API密钥，请设置环境变量 TAVILY_API_KEY。"
    ...
    try:
        ...
        return f"错误：执行Travily搜索时出现问题 - {e}"
...
# 配置LLM客户端
API_KEY = "sk-wtMaIaWLIA6X4K7JIipZPhXU5jqXm6SyC8Fvv0aA7G6pH"
BASE_URL = "http://127.0.0.1:8317/v1"
MODEL_ID = "gemini-3-flash-preview"
TAVILY_API_KEY = "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"
...
for i in range(5):  # 设置最大循环次数
    ...
    if match:
        truncated_output = match.group(1).strip()
