...
# Patch QdrantClient for compatibility
if not hasattr(QdrantClient, 'search'):
    print("Patching QdrantClient.search...")
    def search(self, collection_name, query_vector, **kwargs):
        return self.query_points(collection_name, query=query_vector, **kwargs).points
    QdrantClient.search = search

# Patch CollectionInfo for compatibility
if not hasattr(models.CollectionInfo, 'vectors_count'):
    print("Patching CollectionInfo.vectors_count...")
    models.CollectionInfo.vectors_count = property(lambda self: self.points_count)
...
def create_gradio_ui():
    ...
    def load_pdf(pdf_file) -> str:
        ...
        # gr.File(type="filepath") 返回的是字符串路径；其他配置下可能是文件对象
    ...
    with gr.Blocks(title="智能文档问答助手", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("🏠 开始使用"):
            ...
            init_btn.click(init_assistant, inputs=[user_id_input], outputs=[init_output], api_name=False)
            ...
            load_btn.click(load_pdf, inputs=[pdf_upload], outputs=[load_output], api_name=False)
        ...
        with gr.Tab("💬 智能问答"):
            ...
            msg_input.submit(chat, inputs=[msg_input, chatbot], outputs=[msg_input, chatbot], api_name=False)
            send_btn.click(chat, inputs=[msg_input, chatbot], outputs=[msg_input, chatbot], api_name=False)
        ...
        with gr.Tab("📝 学习笔记"):
            ...
            note_btn.click(add_note_ui, inputs=[note_content, concept_input], outputs=[note_output], api_name=False)

        with gr.Tab("📊 学习统计"):
            gr.Markdown("### 查看学习进度和统计信息")
            stats_btn = gr.Button("刷新统计", variant="primary")
            stats_output = gr.Markdown()
            stats_btn.click(get_stats_ui, outputs=[stats_output], api_name=False)

            gr.Markdown("### 生成学习报告")
            report_btn = gr.Button("生成报告", variant="primary")
            report_output = gr.Textbox(label="报告状态", interactive=False)
            report_btn.click(generate_report_ui, outputs=[report_output], api_name=False)
