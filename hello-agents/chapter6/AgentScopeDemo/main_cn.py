    ...
    async def werewolf_phase(self, round_num: int):
        ...
        async with MsgHub(
            ...
            announcement=await self.moderator.announce(
                f"狼人们请相互配合，讨论今晚的击杀目标。存活玩家：{format_player_list(self.alive_players)}"
            ),
        ) as werewolves_hub:
            # 讨论阶段
            ...
            for i, vote_msg in enumerate(kill_votes):
                ...
                if vote_msg is not None and hasattr(vote_msg, 'metadata') and vote_msg.metadata is not None:
                    ...
                    print(f"🗳️ {self.werewolves[i].name} 投票击杀 {votes[self.werewolves[i].name]}")
                else:
                    # 如果返回无效,随机选择一个目标
        ...
        async with MsgHub(
            ...
            for i, vote_msg in enumerate(vote_msgs):
                ...
                if vote_msg is not None and hasattr(vote_msg, 'metadata') and vote_msg.metadata is not None:
                    ...
                    print(f"🗳️ {self.alive_players[i].name} 投票淘汰 {votes[self.alive_players[i].name]}")
                else:
                    # 如果返回无效,默认弃票
